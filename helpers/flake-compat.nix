  ...
  fetchTree =
    builtins.fetchTree or (
      ...
      if info.type == "github" then
          ...
          outPath = fetchTarball (
            {
              url =
                baseProxyUrl
                + "https://api.${info.host or "github.com"}/repos/${info.owner}/${info.repo}/tarball/${info.rev}";
            }
        ...
        {
          outPath = fetchTarball (
            { url = baseProxyUrl + info.url; } // (if info ? narHash then { sha256 = info.narHash; } else { })
          );
        }
          ...
          outPath = fetchTarball (
            {
              url =
                baseProxyUrl
                + "https://${info.host or "gitlab.com"}/api/v4/projects/${info.owner}%2F${info.repo}/repository/archive.tar.gz?sha=${info.rev}";
            }
            // (if info ? narHash then { sha256 = info.narHash; } else { })
          );
          shortRev = builtins.substring 0 7 info.rev;
          ...
          outPath = fetchTarball (
            {
              url =
                baseProxyUrl
                + "https://${info.host or "git.sr.ht"}/${info.owner}/${info.repo}/archive/${info.rev}.tar.gz";
            }
            // (if info ? narHash then { sha256 = info.narHash; } else { })
          );
          shortRev = builtins.substring 0 7 info.rev;
